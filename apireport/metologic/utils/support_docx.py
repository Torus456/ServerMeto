...
def create_docx64(data_js):
    ...
    fldChar = parse_xml(r'<w:fldChar {} w:fldCharType="begin"/>'.format(nsdecls('w')))  # creates a new element
    # fldChar.set(nsdecls('w:fldCharType'), 'begin')  # sets attribute on element
    instrText = parse_xml(r'<w:instrText {} xml:space="preserve"/>'.format(nsdecls('w')))
    # instrText.set(nsdecls('xml:space'), 'preserve')  # sets attribute on element
    instrText.text = 'TOC \\o "1-9" \\h \\z \\u'   # change 1-3 depending on heading levels you need
