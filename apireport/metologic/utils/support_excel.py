...
def fill_excel_for_krost(data_js):
    """ По данным сформируем excel файл"""
    os.environ["NLS_LANG"] = '.AL32UTF8'
    sql_path = settings.BASE_DIR + "/supp/sqlscript/67/"
    con = cx_Oracle.connect('CS_ART/CS_ART@192.168.54.17:1521/ORA5.INCON.LO')
    ...
    df_cls = df_cls.loc[df_cls["ISLEAF"] == 1]
    df_cls = df_cls.loc[df_cls["CNTOBJ"].notnull()]
    for index, row in df_cls.iterrows():
        project_args_obj["CLS_ID"] = row["CLS_ID"]
        project_args_obj["CLF_ID"] = row["CLF_ID"]
        columns_start = [
        ...
        columns_end = ['Замечания']
        print(row["SNAME"])
        mapping = mapping_sdv(row["MAPFNAME"])
        mapping["Статус записи"] = "Статус записи (1 - эталонная, 0  - неполная)"
        ...
        print("Запрос")
        if len(mapping) <= 1:
            df_vsn = df_vsn[columns_start + columns_end]
        else:
            df_vsn = df_vsn[columns_start + list(mapping) + columns_end]
        df_vsn = df_vsn.drop_duplicates()
        df_vsn = df_vsn.rename(columns=mapping)
    ...
    writer.save()
    result["path_file"] = path_file
    result["name"] = str("KROST_" + str(datetime.now().strftime("%Y-_%m-%d-%H_%M_%S")))
    return result
...
def fill_excel_for_ns(data_js):
    """ По данным сформируем excel файл"""
    os.environ["NLS_LANG"] = '.AL32UTF8'
    sql_path = settings.BASE_DIR + "/supp/sqlscript/62/"
    con = cx_Oracle.connect('CS_ART/CS_ART@192.168.54.17:1521/ORA5.INCON.LO')
    result = {}
    project_args = {
        "MLT_ID": data_js.get("project_args").get("mlt_id"),
    ...
    temp_dict = {}
    temp_dict["catalog"] = []
    path_file = settings.BASE_DIR + "/upload/uni_" + "_" + str(datetime.now().strftime("%Y-_%m-%d-%H_%M_%S")) + ".xlsx"
    writer = pd.ExcelWriter(path_file)
    ...
    for index, row in df_cls.iterrows():
        ...
        columns_start = [
            'MLT_ID', 'CLF_ID', 'CLS_ID', 'OBJ_ID',
            'Наименование историческое', 'Код класса', 'Класс',
        ]
        columns_end = [
            'Наименования полное', 'Наименования краткое'
        ]
        mapping = mapping_sdv(row["MAPFNAME"])
        df_vsn = fill_dataframe(
            sql_path,
    ...
    writer.save()
    result["path_file"] = path_file
    result["name"] = str("UNI_" + str(datetime.now().strftime("%Y-_%m-%d-%H_%M_%S")))
    return result
