...
def get_data_cls(cls, dvs, obj, vsn, lst):
    ...
    data_cls["basic_attributes"] = get_attributes(cls, dvs, lst, 1)
    # Атрибуты поставщика
    data_cls["supplier_attributes"] = get_attributes(cls, dvs, lst, 0, True)
    # Еще какие-то атрибуты
    data_cls["auxiliary_attributes"] = get_attributes(cls, dvs, lst, 0, False)
    # Продуктовые карточки
    data_cls["product_reference_cards"] = get_products(cls, obj, vsn)
    return data_cls


def get_attributes(cls, dvs, lst, type, isSupplier=False):
    ...
    for index, row in df_attributes.iterrows():
        ...
        if row["VALTYPE"] == "Список":
            df_lst = lst.loc[lst["OD"] == row["NAME"]]
            for ind, rowlst in df_lst.iterrows():
                sdv["available_values"].append(rowlst["VALCHAR"])
